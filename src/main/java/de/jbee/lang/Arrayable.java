 ...
 * moved to the caller. As an additional benefit it can be avoided to create temporary arrays just
 * to 'get out' the data before copying multiple parts together into a larger one.
 * </p>
 * 
...
public interface Arrayable {
	 ...
	 * <code>offset</code> position of the <code>dest</code> array. It should fill (up to)
	 * <code>length</code> data form its <code>start</code> index.
	 */
	void fill( int offset, Object[] dest, int start, int length );
