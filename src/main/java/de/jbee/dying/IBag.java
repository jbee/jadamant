...
public interface IBag<T, C extends IBag<T, C>>
	...
	boolean isEqualBy( Eq<? super T> equality, C other );
	 ...
	 * Counts the occurrences of the element e in this bag.
	 */
	int countBy( Eq<? super T> equality, T e );
	 ...
	 * equality test
	 */
	C deleteBy( Eq<? super T> equality, T e );

	C filter( Predicate<? super T> filterFunction );

	C intersect( ICluster<T> other );

	C intersectBy( Eq<? super T> equality, ICluster<T> other );

	C union( ICluster<T> other );

	C unionBy( Eq<? super T> equality, ICluster<T> other );

	C difference( ICluster<T> other );

	C differenceBy( Eq<? super T> equality, ICluster<T> other );
